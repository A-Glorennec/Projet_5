...
async function submitOrder(contact) {
    let products = getSelectedProductId(cartToOrder);
    let finalOrderObject = { contact, products };
    let orderId = await sendOrderRequest(finalOrderObject);
    redirectToConfirmationPage(orderId);
}


// Fonction pour récupérer les ids des produits
function getSelectedProductId (){
    // Récupération du panier depuis le localStorage
    cart = getCartFromLocalStorage();
    // Création du tableau de produits vide 
    let products = [];
...
async function sendOrderRequest(finalOrderObject) {
    const orderIdResponse = await fetch("http://localhost:3000/api/products/order", {
        method : "POST",
...
// Fonction pour rediriger vers la confirmation de commande
